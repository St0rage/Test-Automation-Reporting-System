...
export class PlainReportBuilder implements IPlainReportBuilder {
  ...
  public async createReport(
    ...
    const beritaAcaraData: BeritaAcaraData = {
      scenarioData: {
        testCaseId: report.test_case.unique_id,
        scenario: report.test_case.scenario.name,
        testCase: report.test_case.name,
        expectedResult: report.test_case.expected_result,
        criteria: report.test_case.criteria,
      },
      documentAttributData: {
        tools: report.project.tool.name,
    ...
    await this.createBeritaAcaraPage2(3, report.test_case.description);
    ...
    fs.writeFile(path.join(reportPath, fileName), Buffer.from(output) as any, (err) => {
      if (err) {
        throw err;
      }
    });
