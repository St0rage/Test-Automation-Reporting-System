...
describe("report-service-save-report-test", () => {
  ...
  test("should save report", async () => {
    ...
    await expect(reportService.saveReport(id)).resolves.toBeUndefined();
    expect(mockDeleteFile).toHaveBeenCalledTimes(1);
  });

  test("should throw error when FAILED step data found in save report", async () => {
    const reportService: IReportService =
      container.resolve<IReportService>(ReportService);

    const id = 2; // Represent as FAILED report

    await expect(reportService.saveReport(id)).rejects.toThrow(ResponseError);
    expect(mockDeleteFile).not.toHaveBeenCalledTimes(1);
  });

  test("should throw error when step data length < 1 in save report", async () => {
    const reportService: IReportService =
      container.resolve<IReportService>(ReportService);
